...
class VideogamesService {
  ...
  async findAll() {
    try {
      ...
      throw boom.notFound("videogames not found");
    }
  }
  ...
  async findByName(name) {
    try {
      ...
      throw boom.notFound(`search for ${name} not found`);
    }
  }
  ...
  async findOne(id) {
    if (typeof id === "string" && id.length === 36) {
      try {
        ...
        throw boom.notFound("Sorry, game info not found");
      }
    } else {
      try {
  ...
  async add(item) {
    ...
    try {
      ...
      throw boom.notFound(`add game not found`);
    }
  }
}
